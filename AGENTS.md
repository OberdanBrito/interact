...
- **Playwright MCP:** a bottom sheet é `<div>` (não `<dialog>`) — seletores `#sheet .js-*`; no QA pode já haver sessão ativa (form de login oculto) — conferir com snapshot antes de tentar logar.
